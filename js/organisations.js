export function createOrganisations({ client, state, escapeHtml: h, safeUrl, avatar, peopleResults, socialRow, setStatus, setSubmitting, navigate, render, competitionList }) {
  ...
  function bind() {
    const directory = document.querySelector('[data-org-search]');
    const invitation = document.querySelector('[data-org-invite]');
    for (const searchForm of [directory, invitation].filter(Boolean)) {
      const isDirectory = directory === searchForm;
      const input = isDirectory ? searchForm.elements.query : searchForm.elements.username;
      const results = document.querySelector(isDirectory ? '[data-org-results]' : '[data-invite-suggestions]');
      const status = document.querySelector(isDirectory ? '[data-org-search-status]' : '[data-suggestion-status]');
      ...
      const run = async () => {
        ...
        if (isDirectory) document.querySelector('.pagination').hidden = query.length >= 2;
        if (query.replace(/^@/, '').length < 2) { results.innerHTML = isDirectory ? original : ''; setStatus(status, query ? 'Enter at least two characters.' : ''); return; }
        setStatus(status, 'Searching…');
        try {
          const rows = await result(client.rpc(isDirectory ? 'search_organisations' : 'search_people', isDirectory ? { search_term: query } : { search_term: query, organiser_only: true }));
          if (current !== revision || !searchForm.isConnected) return;
          setStatus(status, `${rows.length} ${isDirectory ? 'organisations' : 'organisers'} found.`);
          results.innerHTML = isDirectory ? rows.map(orgLink).join('') : rows.map(p => `<button type="button" class="person-row suggestion" data-select-organiser="${h(p.username)}">${avatar(p,'avatar-small')}<span><strong>${h(p.full_name)}</strong><small>@${h(p.username)}</small></span></button>`).join('');
          results.querySelectorAll('[data-select-organiser]').forEach(button => button.addEventListener('click', () => { ++revision; clearTimeout(timer); input.value = '@' + button.dataset.selectOrganiser; results.innerHTML = ''; setStatus(status, 'Organiser selected. Send the invitation when ready.'); searchForm.querySelector('[data-submit]').focus(); }));
        } catch(error) { if (current === revision && searchForm.isConnected) { results.innerHTML = ''; setStatus(status, message(error), 'error'); } }
      };
      input.addEventListener('input', () => { ++revision; clearTimeout(timer); timer = setTimeout(run, 220); });
      input.addEventListener('keydown', e => { if(e.key === 'Escape') { ++revision; clearTimeout(timer); results.innerHTML=''; } });
      if (isDirectory) searchForm.addEventListener('submit', e => { e.preventDefault(); clearTimeout(timer); run(); });
    }
